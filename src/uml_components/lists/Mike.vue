<template>
  <div>
    <div id="main">Mike's List</div>
    <Form @mikeFormSubmit="addToList" />
    ...
    <button v-if="this.display === 'uml'" @click="fetchVinyl">
      Click to view Mike's Vinyl Collection
    </button>
    <button v-else-if="this.display === 'vinyl'" @click="fetchFavorites">
      Click to view Mike's Ultimate Music List
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    fetchFavorites() {
      axios
        .get("https://vue-umlcc-mike-default-rtdb.firebaseio.com/uml.json")
        .then((response) => {
    ...
    addToList(artist) {
      if (this.display === "uml") {
        axios
          .post(
            `https://vue-umlcc-mike-default-rtdb.firebaseio.com/uml.json/?auth=${localStorage.getItem(
              "idToken"
            )}`,
        ...
        axios
          .post(
            `https://vue-umlcc-mike-default-rtdb.firebaseio.com/vinyl/.json/?auth=${localStorage.getItem(
              "idToken"
            )}`,
            JSON.stringify(artist)
          )
    ...
    fetchVinyl() {
      axios
        .get("https://vue-umlcc-mike-default-rtdb.firebaseio.com/vinyl.json")
        .then((response) => {
          this.fetchedList = response.data;
          this.display = "vinyl";
        })
    ...
    deleteItem(deleteArtist) {
      console.log('mike here', deleteArtist);
    }
  },
  mounted() {
    this.fetchFavorites();
