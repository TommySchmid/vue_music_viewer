<template>
  <div>
    ...
    <button v-if="this.display === 'uml'" @click="fetchVinyl">
      Click to view Kayla's Vinyl Collection
    </button>
    <button v-else-if="this.display === 'vinyl'" @click="fetchFavorites">
      Click to view Kayla's Ultimate Music List
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    fetchFavorites() {
      axios
        .get("https://vue-umlcc-kayla-default-rtdb.firebaseio.com/uml.json")
        .then((response) => {
    ...
    addToList(artist) {
      if (this.display === "uml") {
        axios
          .post(
            `https://vue-umlcc-kayla-default-rtdb.firebaseio.com/uml.json/?auth=${localStorage.getItem(
              "idToken"
            )}`,
        ...
        axios
          .post(
            `https://vue-umlcc-kayla-default-rtdb.firebaseio.com/vinyl/.json/?auth=${localStorage.getItem(
              "idToken"
            )}`,
            JSON.stringify(artist)
          )
    ...
    fetchVinyl() {
      axios
        .get("https://vue-umlcc-kayla-default-rtdb.firebaseio.com/vinyl.json")
        .then((response) => {
          this.fetchedList = response.data;
          this.display = "vinyl";
        })
    ...
    deleteItem(deleteArtist) {
      console.log('kayla here', deleteArtist);
    }
  },
  mounted() {
    this.fetchFavorites();
